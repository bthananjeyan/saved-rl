...
class ReacherSparseConfigModule:
    ENV_NAME = "MBRLReacherSparse3D-v0"
    TASK_HORIZON = 100
    NTRAIN_ITERS = 100
    NROLLOUTS_PER_ITER = 1
    PLAN_HOR = 25
    MODEL_IN, MODEL_OUT = 24, 17
    VALUE_IN, VALUE_OUT = 17, 1
    CLONE_IN, CLONE_OUT = 17, 7
    ...
    DEMO_LOW_COST = 70
    DEMO_HIGH_COST = 300
    DEMO_LOAD_PATH = "experts/reachersparse/expert4/logs.mat"
    LOAD_SAMPLES = True
    GYM_ROBOTICS = False
    ...
    def obs_cost_fn(self, obs):
        if isinstance(obs, np.ndarray):
            return (np.sum(np.square(ReacherConfigModule.get_ee_pos(obs, are_tensors=False) - self.ENV.goal), axis=1) > THRESHOLD).astype(np.float32)
        else:
            return tf.cast(tf.reduce_sum(tf.square(ReacherSparseConfigModule.get_ee_pos(obs, are_tensors=True) - self.goal), axis=1) > THRESHOLD, tf.float32)
...
CONFIG_MODULE = ReacherSparseConfigModule
