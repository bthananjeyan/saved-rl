...
from dmbrl.modeling.models import NN, BNN, TFGP, Linear


def create_config(env_name, ctrl_type, ctrl_args, overrides, logdir, mode=None):
    cfg = DotMap()
    type_map = DotMap(
        exp_cfg=DotMap(
            sim_cfg=DotMap(
    ...
    if mode is None:
        ...
        cfg_module = cfg_source.CONFIG_MODULE(mode)
    ...
    _create_val_config(cfg.val_cfg, cfg_module, ctrl_args)
...
def _create_exp_config(exp_cfg, cfg_module, logdir, type_map):
    ...
    exp_cfg.sim_cfg.env.gym_robotics = cfg_module.GYM_ROBOTICS
    ...
    exp_cfg.exp_cfg.num_demos = cfg_module.NUM_DEMOS
    exp_cfg.exp_cfg.demo_low_cost = cfg_module.DEMO_LOW_COST
    exp_cfg.exp_cfg.demo_high_cost = cfg_module.DEMO_HIGH_COST
    exp_cfg.exp_cfg.demo_load_path = cfg_module.DEMO_LOAD_PATH
    exp_cfg.exp_cfg.load_samples = cfg_module.LOAD_SAMPLES
    exp_cfg.exp_cfg.gym_robotics = cfg_module.GYM_ROBOTICS
    exp_cfg.exp_cfg.ss_buffer_size = cfg_module.SS_BUFFER_SIZE
    exp_cfg.exp_cfg.use_value = cfg_module.USE_VALUE

    exp_cfg.log_cfg.logdir = logdir

def _create_val_config(val_cfg, cfg_module, ctrl_args):
    val_cfg.env = cfg_module.ENV
    if hasattr(cfg_module, "UPDATE_FNS"):
        val_cfg.update_fns = cfg_module.UPDATE_FNS
    if hasattr(cfg_module, "obs_preproc"):
        val_cfg.obs_preproc = cfg_module.obs_preproc
    if hasattr(cfg_module, "obs_postproc"):
        val_cfg.obs_postproc = cfg_module.obs_postproc
    if hasattr(cfg_module, "obs_postproc2"):
        val_cfg.obs_postproc2 = cfg_module.obs_postproc2
    if hasattr(cfg_module, "targ_proc"):
        val_cfg.targ_proc = cfg_module.targ_proc

    model_init_cfg_val = val_cfg.model_init_cfg_val
    model_init_cfg_val.num_nets = 5
    model_init_cfg_val.model_constructor = cfg_module.value_nn_constructor
    model_init_cfg_val.model_class = BNN
    val_cfg.model_train_cfg = cfg_module.NN_TRAIN_CFG
    val_cfg.val_buffer_size = cfg_module.VAL_BUFFER_SIZE
    val_cfg.gym_robotics = cfg_module.GYM_ROBOTICS
...
def _create_ctrl_config(ctrl_cfg, cfg_module, ctrl_type, ctrl_args, type_map):
    ...
    """
    if ctrl_type == "MPC":
        ctrl_cfg.alpha_thresh = cfg_module.ALPHA_THRESH
        ctrl_cfg.has_constraints = cfg_module.HAS_CONSTRAINTS
        ctrl_cfg.beta_thresh = cfg_module.BETA_THRESH
        ctrl_cfg.gym_robotics = cfg_module.GYM_ROBOTICS
